...
if (!THOR_API_KEY) {
  console.warn(
    "⚠️ THOR_API_KEY is not set. API routes will reject requests in production until you configure this."
  );
}
...
app.post("/api/health", requireApiKey, upload.none(), async (req, res) => {
  try {
    ...
    const data = {
      ...
      energy_level: energy ? parseInt(energy, 10) : null,
      mood_score: mood ? parseInt(mood, 10) : null,
      libido_score: libido ? parseInt(libido, 10) : null,
    };
  ...
  try {
    ...
    const data = {
      ...
      duration_minutes: duration_minutes
        ...
        : null,
      intensity_level: intensity_level ? parseInt(intensity_level, 10) : null,
      recovery_score: recovery_score ? parseInt(recovery_score, 10) : null,
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      if (insertError) {
        console.error("Supabase error lab_results insert:", insertError);
        return res
          .status(500)
